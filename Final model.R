...
for (j in 1:ncol(data)) {
  windows()
  boxplot(data[, j], pch = 19, xlab = names(data)[j], main = "")
}
...
reg <- lm(service ~ end_of_month + weekend + weather_main_Rain + end_of_month_and_weekend +
  lag_1_service + weekday_Saturday + humidity + weekday_Wednesday +
  weekday_Tuesday + weekday_Thursday, data = data)
summary(reg)
